...
$.SvgCanvas = function(container, config, scope) {
    ...
    var recalculateDimensions = this.recalculateDimensions = function(selected) {
        ...
        switch (selected.tagName) {
            ...
            default:
                if (
                    (tlist.numberOfItems === 1 && tlist.getItem(0).type === 1) || (tlist.numberOfItems === 2 && tlist.getItem(0).type === 1 && tlist.getItem(0).type === 4)
                ) {
                    return null;
                }
        }
    ...
    var textActions = canvas.textActions = function() {
        ...
        return {
            ...
            mouseUp: function(evt, mouse_x, mouse_y) {
                ...
                if (
                    evt.target !== curtext && mouse_x < last_x + 2 && mouse_x > last_x - 2 && mouse_y < last_y + 2 && mouse_y > last_y - 2) {
    ...
    this.customImport = function(svgImage) {
        try {

            // convert string into XML documentc

            // var newDoc = svgedit.utilities.text2xml(xmlString);
            // console.log(newDoc)
                // var newDoc = svgedit.utilities.text2xml(svgImage);
            svgedit.sanitize.sanitizeSvg(svgImage);

            // convert paths into absolute commands
            var paths = svgImage.getElementsByTagNameNS(svgns, "path");
            for (var i = 0, len = paths.length; i < len; ++i) {
                var path = paths[i];
                path.setAttribute('d', pathActions.convertPath(path));
                pathActions.fixEnd(path);
            ...
            if (svgdoc.adoptNode) {
                ...
                console.log('ok')
                svgcontent = svgdoc.adoptNode(svgImage);
            } else {
                console.log('ok')
                svgcontent = svgdoc.importNode(svgImage, true);
            }
